...
The candidate began with Wi-Fi recovery, then added the other components in separate commits. The reviewed extraction removes 1,279 desktop lines against `350c4655`; including the separate audio-restart fix, guards and regression tests, the net reduction is 1,123 lines. Those counts exclude package source, plans and documentation. An upstream desktop submission must omit `packages/`; shared desktop interfaces, package source, migrations and recipes remain separately reviewable.
