...
All three candidate artifacts were built from the same recorded source. Standalone behavioral tests and fresh, full-baseline upgrade and repeated pacman transactions pass with dependency checks enabled, one owner for each transferred file, and no overwrite workaround. The desktop delta is 1,334 lines smaller after accounting for wrappers, integration calls and migration coverage. The non-root desktop aggregate passes CLI and 285/286 shell test files; the remaining optional-menu drift test also fails on the untouched baseline. See the [candidate validation record](https://github.com/omacom/omarchy-mac/blob/quattro-upstream/plans/omarchy-mac-package-validation.md) for exact revisions and limits.

The artifacts are unsigned local candidates. Publication, repository trust and the active desktop remain untouched; installation hooks, signed repository selection and physical M1/M2 qualification are still release gates. Aurora needs separately recorded evidence. Desktop bindings, trackpad/Electron/ALS behavior, HID/initramfs, boot, snapshots and installer work stay outside this add-on release.
